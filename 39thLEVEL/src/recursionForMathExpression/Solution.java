package recursionForMathExpression;
...
public class Solution {
	...
	public void recursion(final String expression, int countOperation) {
		...
		// после вычисления подвыражения, проверка и вычисление синуса, косинуса или
		// тангенса:
		...
		if (expression.contains("+") || expression.contains("-") || expression.contains("*") || expression.contains("/")
				|| expression.contains("^") || expression.contains("sin") || expression.contains("cos")
				|| expression.contains("tan")) {
			if ((trigoStr != null) && (expression.substring(openBracketPos - 3, openBracketPos).equals(trigoStr))) {
				newExpression = expression.substring(0, openBracketPos - 3) + subExpressionResult
				...
				if (openBracketPos == 0 && closeBracketPos == 0) {
					...
					newExpression = expression.substring(0, openBracketPos) + subExpressionResult
							+ (closeBracketPos == expression.length() - 1 ? ""
